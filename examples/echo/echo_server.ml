let msgs = [
  ...
  "YOU are breathtaking!";
  "YOU are breathtaking!";
  "It was fun, let's stop playing now";
...
let _ =
  ...
  let connection_server = Websocketml.Http.create inet_addr in

  (* wait for a connexion *)
  let client_sock, client_addr = Websocketml.Http.listen_and_accept connection_server in
  ...
  begin
    try
      ...
      begin match msg with
        ...
        | Some msg ->
          (* print the message *)
          Websocketml.Logger.info (fun m -> m "Received message: %s"
      ...
      Websocketml.Logger.info (fun m -> m "Sent message : %s" to_send);
    ...
    with e -> Websocketml.Logger.error (fun m -> m "Exception : %s" (Printexc.to_string e))
  end;
  Websocketml.Http.close connection_server
